...
String PPPClass::getIPAddress() const {
  if (_esp_netif == NULL) {
    return "Không có esp_netif";
  }

  esp_netif_ip_info_t ip_info;
  esp_err_t err = esp_netif_get_ip_info(_esp_netif, &ip_info);
  if (err != ESP_OK) {
    log_e("Không thể lấy thông tin IP: %d %s", err, esp_err_to_name(err));
